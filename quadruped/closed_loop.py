...
class QuadrupedLeg:
    ...
    def calculate_ik(self, x, z):
        ...
        # Angle of the hip
        alpha = math.acos(
            (self.L1**2 + distance_to_target**2 - self.L2**2) / (2 * self.L1 * distance_to_target)
        )
...
class ClosedLoopQuadruped:
    def __init__(self):
        ...
        self.legs = {
            "FL": QuadrupedLeg(
                "Front-Left", self.HIP_LENGTH, self.KNEE_LENGTH, is_front=True, is_left=True
            ),
            "FR": QuadrupedLeg(
                "Front-Right", self.HIP_LENGTH, self.KNEE_LENGTH, is_front=True, is_left=False
            ),
            "BL": QuadrupedLeg(
                "Back-Left", self.HIP_LENGTH, self.KNEE_LENGTH, is_front=False, is_left=True
            ),
            "BR": QuadrupedLeg(
                "Back-Right", self.HIP_LENGTH, self.KNEE_LENGTH, is_front=False, is_left=False
            ),
        }
        ...
        try:
            while True:
                ...
                for leg_name, leg in self.legs.items():
                    ...
                    x_corr, z_corr = self.apply_posture_correction(
                        leg, x_base, z_base, pitch_correction, roll_correction
                    )
                ...
                # 5. SEND TO MOTORS
                print(
                    f"IMU(P:{current_pitch:5.1f} R:{current_roll:5.1f}) | "
